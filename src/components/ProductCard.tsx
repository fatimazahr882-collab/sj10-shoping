...
// --- SPEED HELPER: Compresses images via Cloudinary URL ---
const getOptimizedUrl = (url: string) => {
    if (!url) return '/placeholder.jpg';
    if (url.includes('cloudinary.com') && url.includes('/upload/')) {
        // Resize to 400px width (good balance for cards), WebP format, Auto quality
...
export default function ProductCard({ product }: { product: Product | null }) {
    ...
    // ✅ NEW: State for loading the image
    const [imgLoading, setImgLoading] = useState(true);

    const productUrl = `/products/${product?.slug || product?.id}`;

    const handleMouseEnter = () => {
        if (product && pathname !== productUrl) {
            router.prefetch(productUrl);
        }
    };

    // Skeleton for card itself if product data is missing
    ...
    return (
        <div className="product-card" onMouseEnter={handleMouseEnter}>
            ...
            <div className="product-card-img-container">
                {/* ✅ LOADER OVERLAY: Shows SjLoader until image is ready */}
                {imgLoading && (
                ...
                <Image 
                    ...
                    className={`main-img ${imgLoading ? 'opacity-0' : 'opacity-100'}`} // Smooth fade in
                    
                    // We keep unoptimized={true} to avoid 500 server error, 
                    // but we used getOptimizedUrl above to ensure it is actually fast.
                    unoptimized={true} 
                    
                    // ✅ Turn off loader when image is done
                    onLoad={() => setImgLoading(false)} 
                />
                
                {/* Second image doesn't need loader logic, it hides behind the first */}
                {firstImage !== secondImage && (
                    <Image src={secondImage} alt={product.title} fill className="img-back" unoptimized={true} />
                )}
            ...
            <style jsx>{`
                ...
                .loader-overlay {
                    ...
                    width: 100%; 
                    height: 100%;
                    background: #f9fafb; /* Light background so card isn't empty */
                    display: flex;
                    align-items: center;
