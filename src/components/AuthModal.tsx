...
export default function AuthModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const triggerSuccessAndClose = (message: string) => {
    ...
    setTimeout(() => {
      setToastMessage('');
      setModalStep('form');
      onClose(); // Close modal and stay on current page!
    }, 1500);
  };
  ...
  const formatPhoneForBackend = (inputPhone: string) => {
    ...
    if (clean.startsWith('03')) {
      return `+92${clean.slice(1)}`; // Converts 03368361990 -> +923368361990
    }
  ...
  return (
    <>
                  ...
                  {!isLoginView && (
                    <>
                      ...
                      <div className="input-group">
                        ...
                        <input type="tel" placeholder="Phone (e.g. 03368361990)" value={phone} onChange={e => setPhone(e.target.value)} maxLength={13} required />
                      </div>
                    </>
                  )}
      ...
      <style jsx>{`
        ...
        .divider { text-align: center; margin: 20px 0; border-bottom: 1px solid #e2e8f0; line-height: 0.1em; }
        ...
        .toggle-text { text-align: center; font-size: 14px; margin-top: 20px; color: #64748b; font-weight: 500; }
