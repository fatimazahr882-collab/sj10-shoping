// src/components/SupplierClientPage.tsx
"use client";
...
// --- CONFIG ---
const API_URL = process.env.NEXT_PUBLIC_PRODUCT_API_URL || 'https://sj10-cart.vercel.app';
...
export default function SupplierClientPage({ supplierId }: { supplierId: string }) {
        ...
        const fetchData = async () => {
            try {
                ...
                const [supRes, prodRes] = await Promise.all([
                    ...
                    // Use explore-feed for safety and proper formatting
                    fetch(`${API_URL}/api/products/explore-feed?supplierId=${supplierId}&limit=100`)
                ]);
    ...
    useEffect(() => {
        const checkStatus = async () => {
            ...
            try {
                const res = await fetch(`${API_URL}/api/social/follow/status/${supplierId}`, {
    ...
    const handleFollow = async () => {
        const token = getToken();
        if (!token) {
            router.push(`/auth?view=login&redirect=${encodeURIComponent(window.location.pathname)}`);
            return;
        }
    ...
    return (
        <div className="page-wrapper">
            <style jsx global>{`
                ...
                .card { background: white; border-radius: 8px; border: 1px solid #eee; overflow: hidden; display: flex; flex-direction: column; transition: transform 0.2s; position: relative; text-decoration: none;}
            ...
            {nextPreloadImage && (
                <div style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', opacity: 0 }}>
                    <Image src={nextPreloadImage} alt="preload" width={1} height={1} unoptimized />
                </div>
            )}
                ...
                <div className="content">
                        ...
                        <div className="header-inner">
                            <div className="profile-img">
                                <Image
                                    ...
                                    alt={supplier.brand_name || "Supplier"}
                                    width={90} height={90}
                                    style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                                    unoptimized
                                    onError={(e) => { (e.target as any).src = PLACEHOLDER_IMG; }}
                                />
                            </div>
                            <div className="info-col">
                                <h1 className="brand-name">{supplier.brand_name || supplier.name}</h1>
                                <div className="meta">
                    ...
                    <div className="toolbar">
                        <div className="search-box">
                            <FaSearch color="#999" />
                            <input type="text" className="search-inp" placeholder="Search store..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                        </div>
                        ...
                        <>
                            ...
                            <div className="latest-wrap">
                                <Link href={`/products/${latestBig.slug}`} className="latest-big">
                                    <Image src={getSafeImage(latestBig.image_urls)} className="p-img" alt={latestBig.title} fill unoptimized style={{objectFit: 'cover'}}/>
                                    <div style={{position: 'absolute', bottom: 0, width: '100%', background: 'linear-gradient(to top, rgba(0,0,0,0.9), transparent)', padding: '16px', color: 'white'}}>
                                        <div style={{fontSize: '14px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}>{latestBig.title}</div>
                                        <div style={{fontWeight: 'bold', fontSize: '18px'}}>{formatPrice(latestBig.price)}</div>
                                    </div>
                                </Link>
                                <div className="latest-stack">
                                    {latestSmall.map(p => (
                                        <Link href={`/products/${p.slug}`} key={p.id} className="latest-small" style={{textDecoration: 'none'}}>
                                            <div style={{position: 'relative', width: 60, height: 60, flexShrink: 0}}>
                                                <Image src={getSafeImage(p.image_urls)} fill style={{objectFit: 'cover', borderRadius: 4}} alt="" unoptimized />
                                            </div>
                    ...
                    <div className="section-h"><FaFire color="red" /> All Products</div>
                    <div className="grid">
                        {gridProducts.map(p => {
                            ...
                            return (
                                <Link href={`/products/${p.slug}`} key={p.id} className="card">
                                    <div className="img-box">
                                        <Image src={getSafeImage(p.image_urls)} className="p-img" alt={p.title} fill unoptimized onError={(e) => { (e.target as any).src = PLACEHOLDER_IMG }} style={{objectFit: 'cover'}}/>
                                    </div>
                                    <div className="info">
